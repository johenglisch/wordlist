...
class WordlistTable(wx.ListCtrl):
    ...
    def find(self):
        ...
        self.lasthit = self.FindItem(-1, self.searchterm, partial=True)
        if self.lasthit == -1:
    ...
    def find_next(self):
        ...
        newhit = self.FindItem(self.lasthit + 1, self.searchterm, partial=True)
        if newhit == -1:
    ...
    def update(self, wordlist=None):
        ...
        if self.wordlist:
            ...
            self.InsertColumn(0, 'word', align, width=120)
            self.InsertColumn(1, 'frequency', wx.LIST_FORMAT_RIGHT, width=90)
            for i in values:
                self.Append(i)
...
class MainWindow(wx.Frame):
    ...
    def init_ui(self):
        ...
        fileopen = menufile.Append(wx.ID_OPEN,
                                   '&Open...\tCtrl-O',
                                   'Open a text file and create a wordlist')
        ...
        tb_save = self.toolbar.AddLabelTool(wx.ID_SAVE,
                                            label='Save',
    ...
    def on_viewtext(self, event):
        ...
        if self.textview:
            ...
            self.textview = TextView(self.wordlist.text, self.filename, self)
